// Offline build plugin manager:
// - "Add plugin" by file/drag-and-drop still works for local plugins.
// - The remote plugin store list is intentionally not populated, so the
//   application never fetches plugin metadata or packages from the internet.
// - No "remove" buttons are rendered because we only show locally installed
//   plugins and plugin management is left to the local file manager.
...
function loadPlugins()
{
    ...
    // Offline build: only keep the locally installed plugins (_pluginsTmp[0]).
    // The second entry is the remote store plugin list; skip it entirely.
    var _pluginsCur = _pluginsTmp[0];
    _pluginsCur["url"] = _pluginsCur["url"].replace(" ", "%20");

    var _plugins = { "url" : _pluginsCur["url"], "pluginsData" : [] };
    ...
    for (var i = 0; i < _len; i++)
    {
        ...
        _pluginsCur["pluginsData"][i]["isSystemInstall"] = true;
